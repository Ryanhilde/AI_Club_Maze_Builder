...
class Maze:
    ...
    def write_svg(self, filename):
        ...
        with open(filename, 'w') as f:
            ...
            """ draw north and west edge """
            for x in range(self.nx):
                if self.cell_at(x, 0).walls['N']:
                    x1, y1, x2, y2 = x * scx, 0 * scy, (x + 1) * scx, 0 * scy
                    write_wall(f, x1, y1, x2, y2)
            for y in range(self.ny):
    ...
    def pick_wall(self, i):
        """ randomly pick coordinates on one of the four edges"""
        switcher = {
            0: [random.randint(0, self.nx - 1), 0],
            1: [random.randint(0, self.nx - 1), self.ny - 1],
            2: [self.nx - 1, random.randint(0, self.ny - 1)],
            3: [0, random.randint(0, self.ny - 1)],
        }
        return switcher.get(i)

    def make_points(self):
        """ pick random start and end points along edges"""
        w_list = [0, 1, 2, 3]
        wall_names = ['N', 'S', 'E', 'W']

        s_wall = w_list[random.randint(0, 3)]
        start = self.pick_wall(s_wall)
        w_list.pop(s_wall)
